...
size_t CalculateTransferMemorySize(const SocketInitConfig& config) {
    const u32 tcp_tx_max = config.tcp_tx_buf_max_size != 0 ? config.tcp_tx_buf_max_size : config.tcp_tx_buf_size;
    const u32 tcp_rx_max = config.tcp_rx_buf_max_size != 0 ? config.tcp_rx_buf_max_size : config.tcp_rx_buf_size;
...
void CloseServiceTraced(AppContext& ctx, const char* name, Service& service) {
    ...
    logger::Log(ctx, "manual_bsd phase=cleanup_%s_begin handle=0x%X own_handle=%u object_id=%u", name, handle, own_handle, object_id);
    ...
    if (own_handle != 0 || object_id != 0) {
        ...
        if (own_handle != 0) {
            logger::Log(ctx, "manual_bsd phase=cleanup_%s_close_handle_begin handle=0x%X", name, handle);
...
struct ManualBsdState {
    ...
    Result Cleanup() {
        ...
        if (transfer_memory_created) {
            logger::Log(ctx, "manual_bsd phase=cleanup_tmem_begin handle=0x%X size=%zu", transfer_memory.handle, transfer_memory.size);
            const Result rc = tmemClose(&transfer_memory);
            logger::Log(ctx, "manual_bsd phase=cleanup_tmem_complete rc=%s handle=0x%X", FormatResult(rc).c_str(), transfer_memory.handle);
...
ScenarioResult FinishWithFailure(ManualBsdState& state, ScenarioResult result, Result rc, const char* phase) {
    result.rc = rc;
    result.detail = std::string(phase) + " failed: " + FormatResult(rc);
    logger::Log(state.ctx, "manual_bsd phase=failure failed_phase=%s rc=%s", phase, FormatResult(rc).c_str());
...
ScenarioResult RunManualBsdLifecycle(AppContext& ctx) {
    ...
    const SocketInitConfig& socket_config = is_application ? config::SocketConfigApplication : config::SocketConfigApplet;
    ...
    logger::Log(
        ctx,
        "manual_bsd phase=begin process_id_rc=%s process_id=0x%016llX program_id_rc=%s program_id=0x%016llX app_mode=%s "
        "bsd_version=%u open_monitor=%u create_tmem=%u register_client=%u start_monitoring=%u clone_root=%u",
    ...
    if (config::ManualBsdCreateTransferMemory) {
        ...
        logger::Log(
            ctx,
            "manual_bsd phase=create_tmem_begin size=%zu tcp_tx=%u tcp_rx=%u tcp_tx_max=%u tcp_rx_max=%u udp_tx=%u udp_rx=%u "
            "sb_efficiency=%u",
    ...
    if (config::ManualBsdStartMonitoring) {
        ...
        rc = serviceDispatchIn(&state.monitor, 1, client_id, .in_send_pid = true);
        logger::Log(
            ctx,
    ...
    if (config::ManualBsdCloneRootSession) {
        logger::Log(ctx, "manual_bsd phase=clone_root_begin root_handle=0x%X", state.root.session);
    ...
    result.success = true;
    result.detail = "register_client=" + std::to_string(config::ManualBsdRegisterClient) + " client_id=" + std::to_string(client_id) +
