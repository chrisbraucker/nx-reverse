...
constexpr const char* StartupLogPath = "sdmc:/nxrv/toolbox/startup.log";
...
void RecordStartupEvent(const char* phase, Result rc, bool has_rc) {
    ...
    if (has_rc) {
        FormatAndEmit("toolbox startup: %s rc=0x%08x", phase, rc);
    } else {
        FormatAndEmit("toolbox startup: %s", phase);
    }
}

void EnableStartupFileLog() {
    mkdir("sdmc:/nxrv", 0777);
    mkdir("sdmc:/nxrv/toolbox", 0777);

    g_startup_file_ready = true;
    AppendStartupFileLine("toolbox startup: file log ready");

    for (size_t i = 0; i < g_startup_event_count; ++i) {
        const StartupEvent& event = g_startup_events[i];
        if (event.has_rc) {
            FormatAndEmit("toolbox startup replay: %s rc=0x%08x", event.phase, event.rc);
        } else {
            FormatAndEmit("toolbox startup replay: %s", event.phase);
        }
    }
}
