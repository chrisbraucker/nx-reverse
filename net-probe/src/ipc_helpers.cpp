...
Result CreateDriverService(Service* driver_service_creator, Service* out_driver_service) {
    ...
    *out_driver_service = {};
    const Result rc = serviceDispatch(driver_service_creator, 0, .out_num_objects = 1, .out_objects = out_driver_service);
...
Result GetNetworkInterfaceList(Service* driver_service, void* buffer, std::size_t buffer_size, std::uint32_t* out_entry_count) {
    ...
    const Result rc =
        serviceDispatchOut(driver_service, 129, entry_count, .buffer_attrs = {OutAutoBuffer}, .buffers = {{buffer, buffer_size}});
...
Result GetStateChangedEvent(Service* driver_service, Handle* out_event_handle) {
    ...
    *out_event_handle = INVALID_HANDLE;
    const Result rc = serviceDispatch(driver_service, 130, .out_handle_attrs = {SfOutHandleAttr_HipcCopy}, .out_handles = out_event_handle);
...
Result GetNetworkInterfaceListUpdatedEvent(Service* driver_service, Handle* out_event_handle) {
    ...
    *out_event_handle = INVALID_HANDLE;
    const Result rc = serviceDispatch(driver_service, 131, .out_handle_attrs = {SfOutHandleAttr_HipcCopy}, .out_handles = out_event_handle);
...
Result OpenNetworkInterfaceRaw(Service* driver_service, const void* buffer, std::size_t buffer_size, Service* out_network_interface) {
...
Result DuplicateNetworkInterface(Service* network_interface, Service* out_duplicate_interface) {
    ...
    *out_duplicate_interface = {};
    const Result rc = serviceDispatch(network_interface, 0, .out_num_objects = 1, .out_objects = out_duplicate_interface);
...
Result GetNetworkInterfaceInfo(Service* network_interface, void* buffer, std::size_t buffer_size) {
    if (network_interface == nullptr || buffer == nullptr || buffer_size == 0) {
        return MAKERESULT(Module_Libnx, LibnxError_BadInput);
    }

    const Result rc = serviceDispatch(network_interface, 128, .buffer_attrs = {OutAutoBuffer}, .buffers = {{buffer, buffer_size}});
    logger::Log("ISfNetworkInterfaceService::GetNetworkInterfaceInfo(size=0x%zX) -> 0x%08X", buffer_size, rc);
    return rc;
}
...
Result GetNetworkInterfaceStateChangedEvent(Service* network_interface, Handle* out_event_handle) {
    ...
    *out_event_handle = INVALID_HANDLE;
    const Result rc =
        serviceDispatch(network_interface, 129, .out_handle_attrs = {SfOutHandleAttr_HipcCopy}, .out_handles = out_event_handle);
...
Result IoctlNetworkInterface(Service* network_interface, std::uint32_t selector) {
    ...
    logger::Log("ISfNetworkInterfaceService::Ioctl(selector=0x%08X) -> 0x%08X", selector, rc);
    return rc;
}

Result IoctlGetHandleNetworkInterface(Service* network_interface, std::uint32_t selector, Handle* out_handle) {
    ...
    *out_handle = INVALID_HANDLE;
    const Result rc =
        serviceDispatchIn(network_interface, 389, selector, .out_handle_attrs = {SfOutHandleAttr_HipcCopy}, .out_handles = out_handle);
    logger::Log("ISfNetworkInterfaceService::IoctlGetHandle(selector=0x%08X) -> 0x%08X", selector, rc);
    if (R_SUCCEEDED(rc)) {
...
Result CallNetworkInterfaceCommand0x80(Service* network_interface, void* buffer, std::size_t buffer_size) {
    if (network_interface == nullptr || buffer == nullptr || buffer_size == 0) {
        return MAKERESULT(Module_Libnx, LibnxError_BadInput);
    }

    const Result rc = serviceDispatch(network_interface, 0x80, .buffer_attrs = {OutAutoBuffer}, .buffers = {{buffer, buffer_size}});
    logger::Log("ISfNetworkInterfaceService::Command0x80(size=0x%zX) -> 0x%08X", buffer_size, rc);
    return rc;
}

Result CallNetworkInterfaceCommand0x82(Service* network_interface, NetworkInterfaceCommandHandleSet* out_result) {
...
Result CallNetworkInterfaceCommand0x83(Service* network_interface, NetworkInterfaceCommandHandleSet* out_result) {
    if (network_interface == nullptr || out_result == nullptr) {
        return MAKERESULT(Module_Libnx, LibnxError_BadInput);
    }
...
Result CreateUserService(Service* bsd_nu, Service* out_user_service) {
    ...
    *out_user_service = {};
    const Result rc = serviceDispatch(bsd_nu, 0, .out_num_objects = 1, .out_objects = out_user_service);
...
Result AssignNetworkInterface(Service* user_service, Handle network_interface_handle, Service* out_assigned_interface) {
...
Result AddSession(Service* assigned_interface, Handle network_interface_handle) {
    ...
    const Result rc = serviceDispatch(assigned_interface, 0, .in_num_handles = 1, .in_handles = {network_interface_handle});
