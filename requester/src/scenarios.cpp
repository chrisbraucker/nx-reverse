...
std::string FormatSocketAddress(const sockaddr_storage &address,
                                socklen_t address_len) {
  std::string detail = "source_addr_len=" + std::to_string(address_len) +
  ...
  if (inet_ntop(AF_INET, &ipv4->sin_addr, ip_buffer, sizeof(ip_buffer)) ==
      nullptr) {
    return detail + " source_ip=<inet_ntop-failed> source_port=" +
           std::to_string(ntohs(ipv4->sin_port));
  }

  return detail + " source_ip=" + ip_buffer +
...
bool ResolveIpv4(AppContext &ctx, const char *host, std::uint16_t port,
                 int socktype, ResolvedEndpoint &out_endpoint,
                 std::string &detail);
...
void ConfigureCurlCommon(CURL *curl, AppContext &ctx) {
  ...
  curl_easy_setopt(curl, CURLOPT_TIMEOUT_MS,
                   static_cast<long>(config::SocketTimeoutMs));
...
bool RunCurlTransfer(AppContext &ctx, const char *scenario_name,
                     const char *url, bool allow_insecure_tls,
                     ScenarioResult &result) {
  if (!ctx.curl_initialized) {
    result.skipped = true;
    result.detail = std::string("curl unavailable: ") +
                    curl_easy_strerror(ctx.curl_global_rc);
    logger::Log(ctx, "scenario=%s skipped: %s", scenario_name,
                result.detail.c_str());
    return true;
  }
  ...
  if (perform_rc != CURLE_OK) {
    result.detail = std::string("curl_easy_perform failed: ") +
                    curl_easy_strerror(perform_rc);
  ...
  result.detail =
      ...
      (effective_url != nullptr ? effective_url : url) +
      " primary_ip=" + (primary_ip != nullptr ? primary_ip : "unknown") +
      " http_code=" + std::to_string(response_code) + " response_preview=" +
      EscapePreview(response.data.data(), response.size, 160);
...
std::string CollectSslVerifyDiagnostics(SslConnection &ssl_connection) {
  ...
  const Result verify_errors_rc = sslConnectionGetVerifyCertErrors(
      &ssl_connection, &reported_error_count, &copied_error_count,
      verify_errors.data(), static_cast<std::uint32_t>(verify_errors.size()));
  const Result verify_error_rc =
      sslConnectionGetVerifyCertError(&ssl_connection);
  ...
  if (R_SUCCEEDED(verify_errors_rc) && reported_error_count > 0) {
    const std::size_t count =
        std::min<std::size_t>(reported_error_count, verify_errors.size());
...
int TraceSetSockOpt(AppContext &ctx, int sockfd, int level, int optname,
                    const void *optval, socklen_t optlen, int &out_errno) {
  logger::Log(
      ctx,
      "setsockopt begin fd=%d level=%d opt=%d opt_name=%s optval=%p optlen=%u",
      sockfd, level, optname, SocketOptionName(level, optname), optval,
  ...
  logger::Log(ctx,
              ...
              "errno=%d detail=%s",
              sockfd, level, optname, SocketOptionName(level, optname), rc,
              saved_errno,
              rc == 0 ? "success" : FormatErrno(saved_errno).c_str());
  return rc;
}

bool TraceSetSockOptReuseAddr(AppContext &ctx, int sockfd,
                              ScenarioResult &result) {
  const int enabled = 1;
  int saved_errno = 0;
  const int rc = TraceSetSockOpt(ctx, sockfd, SOL_SOCKET, SO_REUSEADDR,
                                 &enabled, sizeof(enabled), saved_errno);
  if (rc != 0) {
    result.err = saved_errno;
    result.detail =
        std::string("SO_REUSEADDR failed: ") + FormatErrno(saved_errno);
    return false;
  }
  return true;
}

bool TraceSetSockOptRecvTimeout(AppContext &ctx, int sockfd,
                                ScenarioResult &result) {
  ...
  int saved_errno = 0;
  const int rc = TraceSetSockOpt(ctx, sockfd, SOL_SOCKET, SO_RCVTIMEO, &timeout,
                                 sizeof(timeout), saved_errno);
  if (rc != 0) {
    result.err = saved_errno;
    result.detail =
        std::string("SO_RCVTIMEO failed: ") + FormatErrno(saved_errno);
    return false;
  }
  return true;
}

bool TraceSetSockOptSendTimeout(AppContext &ctx, int sockfd,
                                ScenarioResult &result) {
  const struct timeval timeout{
      .tv_sec = static_cast<long>(config::SocketTimeoutMs / 1000U),
      .tv_usec = static_cast<long>((config::SocketTimeoutMs % 1000U) * 1000U),
  ...
  int saved_errno = 0;
  const int rc = TraceSetSockOpt(ctx, sockfd, SOL_SOCKET, SO_SNDTIMEO, &timeout,
                                 sizeof(timeout), saved_errno);
  if (rc != 0) {
    result.err = saved_errno;
    result.detail =
        std::string("SO_SNDTIMEO failed: ") + FormatErrno(saved_errno);
    return false;
  }
  return true;
}
...
bool ConnectResolvedTcp(AppContext &ctx, const char *host, std::uint16_t port,
                        ConnectedTcpSocket &out_socket, std::string &detail) {
  ...
  if (connect(sockfd, reinterpret_cast<const struct sockaddr *>(&endpoint.addr),
              endpoint.addr_len) != 0) {
...
bool SendTcpPayload(int sockfd, const void *payload, std::size_t payload_size,
                    std::size_t &bytes_sent, std::string &detail) {
...
bool ReceiveTcpPayload(int sockfd,
                       std::array<char, config::ReadBufferSize> &buffer,
                       std::size_t &bytes_received, std::string &detail) {
...
bool ResolveIpv4(AppContext &ctx, const char *host, std::uint16_t port,
  ...
  if (gai_rc != 0) {
    ...
    logger::Log(ctx, "resolve host=%s port=%u socktype=%d failed: %s", host,
                port, socktype, detail.c_str());
    return false;
  }

  for (const struct addrinfo *it = results; it != nullptr; it = it->ai_next) {
    if (it->ai_family != AF_INET ||
        it->ai_addrlen > sizeof(out_endpoint.addr)) {
      continue;
    }
    ...
    if (inet_ntop(AF_INET, &sin->sin_addr, ip_buffer, sizeof(ip_buffer)) ==
        nullptr) {
  ...
  logger::Log(ctx, "resolve host=%s port=%u produced no AF_INET result", host,
              port);
  return false;
}
...
ScenarioResult RunEnvironmentSnapshot(AppContext &ctx) {
  ...
  if (!ctx.nifm_initialized) {
    ...
    logger::Log(ctx, "scenario=%s skipped: %s", result.name.c_str(),
                result.detail.c_str());
    return result;
  }
  ...
  ctx.env.internet_status_rc = nifmGetInternetConnectionStatus(
      &ctx.env.connection_type, &ctx.env.wifi_strength,
  ...
  ctx.env.ip_config_rc = nifmGetCurrentIpConfigInfo(
      &ctx.env.current_ip, &ctx.env.subnet_mask, &ctx.env.gateway,
      &ctx.env.primary_dns, &ctx.env.secondary_dns);
  ...
  result.detail =
      ...
      " connection_status=" +
      FormatInternetConnectionStatus(ctx.env.connection_status) +
  ...
  logger::Log(ctx, "scenario=%s %s", result.name.c_str(),
              result.detail.c_str());
  return result;
}
...
ScenarioResult RunDnsResolve(AppContext &ctx) {
  ...
  struct addrinfo *results = nullptr;
  const int gai_rc =
      getaddrinfo(config::DnsHostname, nullptr, &hints, &results);
  if (gai_rc != 0) {
    result.err = gai_rc;
    result.detail = std::string("getaddrinfo failed: ") + gai_strerror(gai_rc);
    logger::Log(ctx, "scenario=%s host=%s failed: %s", result.name.c_str(),
                config::DnsHostname, result.detail.c_str());
    return result;
  }
  ...
  result.detail =
      "addresses=" + (addresses.empty() ? std::string("<none>") : addresses);
  logger::Log(ctx, "scenario=%s host=%s %s", result.name.c_str(),
              config::DnsHostname, result.detail.c_str());
  return result;
}
...
ScenarioResult RunPlainTcpConnect(AppContext &ctx) {
  ...
  logger::Status(ctx, "Connecting TCP to %s:%u", config::TcpHost,
                 config::TcpPort);

  ConnectedTcpSocket connection{};
  if (!ConnectResolvedTcp(ctx, config::TcpHost, config::TcpPort, connection,
                          result.detail)) {
    return result;
  }

  std::array<char, config::ReadBufferSize> buffer{};
  if (!SendTcpPayload(connection.sockfd, config::TcpPayload,
                      std::strlen(config::TcpPayload), result.bytes_sent,
                      result.detail)) {
    result.err = errno;
  ...
  if (!ReceiveTcpPayload(connection.sockfd, buffer, result.bytes_received,
                         result.detail)) {
    result.err = errno;
    close(connection.sockfd);
    return result;
  ...
  result.detail =
      "connected_ip=" + connection.ip + " request_preview=" +
      EscapePreview(config::TcpPayload, std::strlen(config::TcpPayload), 96) +
      " response_preview=" +
      EscapePreview(buffer.data(), result.bytes_received, 160);
  close(connection.sockfd);
  return result;
}
...
ScenarioResult RunIdleTcpHold(AppContext &ctx) {
  ...
  logger::Status(ctx, "Holding idle TCP socket to %s:%u for %u ms",
                 config::TcpHost, config::TcpPort, config::IdleSocketHoldMs);
...
ScenarioResult RunHttpGet(AppContext &ctx) {
  ...
  logger::Status(ctx, "Running HTTP GET for http://%s%s", config::HttpHost,
                 config::HttpPath);

  ConnectedTcpSocket connection{};
  if (!ConnectResolvedTcp(ctx, config::HttpHost, config::HttpPort, connection,
                          result.detail)) {
    return result;
  }
  ...
  const int request_len =
      std::snprintf(request, sizeof(request),
                    "GET %s HTTP/1.1\r\nHost: %s\r\nUser-Agent: "
                    "nxrv-requester/1\r\nConnection: close\r\n\r\n",
                    config::HttpPath, config::HttpHost);
  if (request_len <= 0 ||
      static_cast<std::size_t>(request_len) >= sizeof(request)) {
    result.detail = "request formatting failed";
    close(connection.sockfd);
    return result;
  }

  const ssize_t send_rc = send(connection.sockfd, request,
                               static_cast<std::size_t>(request_len), 0);
  if (send_rc < 0) {
    result.err = errno;
  ...
  std::array<char, config::ReadBufferSize> buffer{};
  const ssize_t recv_rc =
      recv(connection.sockfd, buffer.data(), buffer.size(), 0);
  if (recv_rc < 0) {
    result.err = errno;
  ...
  result.detail =
      "connected_ip=" + connection.ip + " request_preview=" +
      EscapePreview(request, static_cast<std::size_t>(request_len), 96) +
      " response_preview=" +
      EscapePreview(buffer.data(), result.bytes_received, 160);
...
ScenarioResult RunHttpsGet(AppContext &ctx) {
  ...
  if (!ctx.ssl_initialized) {
    ...
    logger::Status(ctx, "Running HTTPS GET for https://%s%s", config::HttpsHost,
                   config::HttpsPath);
    logger::Log(ctx, "scenario=%s skipped: %s", result.name.c_str(),
                result.detail.c_str());
    return result;
  ...
  ResolvedEndpoint endpoint{};
  if (!ResolveIpv4(ctx, config::HttpsHost, config::HttpsPort, SOCK_STREAM,
  ...
  const int transferred_sockfd =
      socketSslConnectionSetSocketDescriptor(&ssl_connection, sockfd);
  ...
  rc = sslConnectionSetHostName(&ssl_connection, config::HttpsHost,
                                std::strlen(config::HttpsHost));
  if (R_FAILED(rc)) {
    result.rc = rc;
  ...
  rc = sslConnectionSetVerifyOption(
      &ssl_connection, SslVerifyOption_PeerCa | SslVerifyOption_HostName |
  ...
  const int request_len =
      std::snprintf(request, sizeof(request),
                    "GET %s HTTP/1.1\r\nHost: %s\r\nUser-Agent: "
                    "nxrv-requester/1\r\nConnection: close\r\n\r\n",
                    config::HttpsPath, config::HttpsHost);
  if (request_len <= 0 ||
      static_cast<std::size_t>(request_len) >= sizeof(request)) {
    result.detail = "request formatting failed";
    if (connection_created) {
  ...
  rc = sslConnectionWrite(&ssl_connection, request,
                          static_cast<std::uint32_t>(request_len), &write_size);
  if (R_FAILED(rc)) {
    result.rc = rc;
  ...
  rc = sslConnectionRead(&ssl_connection, buffer.data(),
                         static_cast<std::uint32_t>(buffer.size()), &read_size);
  if (R_FAILED(rc)) {
    result.rc = rc;
  ...
  const std::string cipher_summary =
      R_SUCCEEDED(rc)
  ...
  result.detail =
      "connected_ip=" + endpoint.ip + " cipher=" + cipher_summary +
      " request_preview=" +
      EscapePreview(request, static_cast<std::size_t>(request_len), 96) +
      " response_preview=" +
      EscapePreview(buffer.data(), result.bytes_received, 160);
...
ScenarioResult RunCurlHttpGet(AppContext &ctx) {
  ...
  char url[512];
  const int url_len =
      std::snprintf(url, sizeof(url), "http://%s:%u%s", config::HttpHost,
                    config::HttpPort, config::HttpPath);
  ...
  static_cast<void>(
      RunCurlTransfer(ctx, result.name.c_str(), url, false, result));
  return result;
}
...
ScenarioResult RunCurlHttpsGet(AppContext &ctx) {
  ...
  char url[512];
  const int url_len =
      std::snprintf(url, sizeof(url), "https://%s:%u%s", config::HttpsHost,
                    config::HttpsPort, config::HttpsPath);
  if (url_len <= 0 || static_cast<std::size_t>(url_len) >= sizeof(url)) {
    result.detail = "url formatting failed";
    return result;
  }
  ...
  static_cast<void>(
      RunCurlTransfer(ctx, result.name.c_str(), url, true, result));
  return result;
}

bool PrepareUdpTarget(AppContext &ctx, ScenarioResult &result,
                      ResolvedEndpoint &endpoint) {
  ...
  return ResolveIpv4(ctx, config::UdpHost, config::UdpPort, SOCK_DGRAM,
                     endpoint, result.detail);
}
...
ScenarioResult RunUdpSendToOnly(AppContext &ctx) {
  ...
  logger::Status(ctx, "Running UDP sendto-only to %s:%u", config::UdpHost,
                 config::UdpPort);
  ...
  if constexpr (config::EnableUdpSendToOnlyTimeouts) {
    ...
    logger::Log(ctx,
                "udp_sendto_only skipping internal SO_RCVTIMEO/SO_SNDTIMEO");
  }

  const ssize_t send_rc =
      sendto(sockfd, config::UdpPayload, std::strlen(config::UdpPayload), 0,
  ...
  result.bytes_sent = static_cast<std::size_t>(send_rc);
  result.detail =
      "target_ip=" + endpoint.ip + " internal_timeouts=" +
      (config::EnableUdpSendToOnlyTimeouts ? "enabled" : "disabled");
  close(sockfd);
  return result;
}
...
ScenarioResult RunUdpConnectSendOnly(AppContext &ctx) {
  ...
  logger::Status(ctx, "Running UDP connect+send-only to %s:%u", config::UdpHost,
                 config::UdpPort);
  ...
  const ssize_t send_rc =
      send(sockfd, config::UdpPayload, std::strlen(config::UdpPayload), 0);
  if (send_rc < 0) {
    result.err = errno;
    result.detail = "send failed: " + FormatErrno(errno);
    close(sockfd);
...
ScenarioResult RunUdpEcho(AppContext &ctx) {
  ...
  logger::Status(ctx, "Running UDP echo to %s:%u", config::UdpHost,
                 config::UdpPort);
  ...
  if constexpr (config::EnableUdpEchoPoll) {
    ...
    const int poll_rc =
        poll(&poll_fd, 1, static_cast<int>(config::SocketTimeoutMs));
    if (poll_rc < 0) {
      result.err = errno;
  ...
  result.success =
      ...
      std::memcmp(buffer.data(), config::UdpPayload, result.bytes_received) ==
          0;
  result.detail =
      "target_ip=" + endpoint.ip + " socket_timeouts=" +
      (config::EnableUdpEchoSocketTimeouts ? "enabled" : "disabled") +
      " poll=" + (config::EnableUdpEchoPoll ? "enabled" : "disabled") + " " +
      source_detail + " response_preview=" +
      EscapePreview(buffer.data(), result.bytes_received, 96);
  close(sockfd);
  return result;
}
...
ScenarioResult RunConcurrentTcpBurst(AppContext &ctx) {
  ...
  logger::Status(ctx, "Running %u concurrent TCP sessions to %s:%u",
                 config::ConcurrentSocketCount, config::TcpHost,
                 config::TcpPort);
  ...
  for (std::size_t i = 0; i < sockets.size(); ++i) {
    ...
    const int payload_len =
        std::snprintf(payload, sizeof(payload), "nxrv-requester-tcp-%zu", i);
    if (payload_len <= 0 ||
        static_cast<std::size_t>(payload_len) >= sizeof(payload)) {
    ...
    std::size_t sent = 0;
    std::string detail;
    if (!SendTcpPayload(sockets[i], payload,
                        static_cast<std::size_t>(payload_len), sent, detail)) {
      result.err = errno;
  ...
  result.success = true;
  result.detail = "connected_ip=" + first_ip +
...
void LogScenarioResult(AppContext &ctx, const ScenarioResult &result) {
  logger::Log(ctx,
              ...
              result.name.c_str(), result.success ? 1 : 0,
              result.skipped ? 1 : 0, FormatResult(result.rc).c_str(),
              FormatErrno(result.err).c_str(), result.bytes_sent,
              result.bytes_received, result.detail.c_str());
}

void RunScenarioStep(AppContext &ctx, std::vector<ScenarioResult> &results,
                     const ScenarioStep &step, const char *next_enabled_name) {
  if (!step.enabled) {
  ...
  if (next_enabled_name != nullptr && config::ScenarioStepDelayMs > 0) {
    logger::Log(ctx, "scenario_pause next=%s duration_ms=%u", next_enabled_name,
                config::ScenarioStepDelayMs);
    SleepMilliseconds(config::ScenarioStepDelayMs);
  }
}

const char *FindNextEnabledScenarioName(const ScenarioStep *steps,
                                        std::size_t count,
...
std::vector<ScenarioResult> RunScenarios(AppContext &ctx) {
  const ScenarioStep steps[] = {
      {"manual_bsd_lifecycle", config::EnableScenarioManualBsdLifecycle,
       RunManualBsdLifecycle},
      {"environment_snapshot", config::EnableScenarioEnvironmentSnapshot,
       RunEnvironmentSnapshot},
      {"dns_resolve", config::EnableScenarioDnsResolve, RunDnsResolve},
      {"plain_tcp_connect", config::EnableScenarioPlainTcpConnect,
       RunPlainTcpConnect},
      ...
      {"udp_socket_only", config::EnableScenarioUdpSocketOnly,
       RunUdpSocketOnly},
      {"udp_socket_setsockopt", config::EnableScenarioUdpSocketSetSockOpt,
       RunUdpSocketSetSockOpt},
      {"udp_setsockopt_reuseaddr", config::EnableScenarioUdpSetSockOptReuseAddr,
      ...
      {"udp_sendto_only", config::EnableScenarioUdpSendToOnly,
       RunUdpSendToOnly},
      {"udp_connect_send_only", config::EnableScenarioUdpConnectSendOnly,
       RunUdpConnectSendOnly},
      {"udp_echo", config::EnableScenarioUdpEcho, RunUdpEcho},
      {"tcp_multi_connect", config::EnableScenarioConcurrentTcpBurst,
       RunConcurrentTcpBurst},
      {"wgnx_packet_udp_echo", config::EnableScenarioWgnxUdpEcho,
       RunWgnxPacketUdpEcho},
      {"wgnx_tunnel_udp_workload", config::EnableScenarioWgnxTunnelUdpWorkload,
  ...
  for (std::size_t i = 0; i < std::size(steps); ++i) {
    RunScenarioStep(ctx, results, steps[i],
