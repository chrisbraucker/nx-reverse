...
ScenarioResult RunUdpEcho(AppContext& ctx) {
    ...
    REQUESTER_LOG_PACKET(ctx, "udp_echo recvfrom bytes_received=%zd %s", recv_rc, source_detail.c_str());
...
ScenarioResult RunConcurrentTcpBurst(AppContext& ctx) {
    ...
    for (std::size_t i = 0; i < sockets.size(); ++i) {
        char payload[64];
        const int payload_len = std::snprintf(payload, sizeof(payload), "nxrv-requester-tcp-%zu", i);
