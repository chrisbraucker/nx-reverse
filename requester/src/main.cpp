...
void LogDiagnostics(requester::AppContext& context, const requester::RuntimeConfig& config, const requester::ConfigLoadReport& report) {
    requester::logger::Log(context, "requester start title=%s version=%s", APP_TITLE, nxrv::build_info::VersionWithBuild);
    requester::logger::Log(context, "hos_version=%s", requester::FormatHosVersion().c_str());
    requester::logger::Log(context, "run_id=%s log_path=%s", context.run_id.c_str(), context.log_path.c_str());
    requester::logger::Log(context, "wgnx_api ctl=%u tun=%u", wgnx::IpcApiVersion, wgnx::tunnel::TunApiVersion);
    ...
    for (const std::string& diagnostic : report.diagnostics) {
        requester::logger::Status(context, "configuration warning: %s", diagnostic.c_str());
    }
}
...
int main(int argc, char** argv) {
    ...
    if (!requester::EnsureLogDirectories() || !requester::EnsureRuntimeConfigDirectories()) {
    ...
    requester::ConfigLoadReport loaded_config = requester::LoadRuntimeConfig(defaults);
    LogDiagnostics(context, loaded_config.config, loaded_config);

    const int rc = requester::RunRequesterUi(context, defaults, std::move(loaded_config));
