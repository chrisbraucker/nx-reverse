...
void PrintSummary(AppContext &ctx, const std::vector<ScenarioResult> &results) {
  for (const auto &result : results) {
    ...
    logger::Status(ctx, "[%s] %s | sent=%zu recv=%zu | %s", state,
...
int main(int argc, char **argv) {
  ...
  logger::Log(ctx, "requester start title=%s version=%s-%s", APP_TITLE, VERSION,
              BUILD_ID);
  logger::Log(ctx, "hos_version=%s", FormatHosVersion().c_str());

  const bool is_application = appletGetAppletType() == AppletType_Application;
  logger::Bootstrap("main: applet type resolved mode=%s",
                    is_application ? "application" : "applet");
  const SocketInitConfig &socket_config = is_application
                                              ? config::SocketConfigApplication
                                              : config::SocketConfigApplet;
  logger::Log(
      ctx,
      "execution_config applet_exit_lock=%u socket=%u nifm=%u ssl=%u curl=%u "
      "curl_share=%u scenario_manual_bsd=%u manual_bsd_open_monitor=%u "
      "manual_bsd_create_tmem=%u manual_bsd_register=%u "
      "manual_bsd_start_monitoring=%u manual_bsd_clone_root=%u "
      "scenario_environment=%u scenario_dns=%u scenario_plain_tcp=%u "
      "scenario_idle_tcp=%u scenario_http=%u scenario_https=%u "
      "scenario_curl_http=%u scenario_curl_https=%u "
      "scenario_udp_socket_only=%u scenario_udp_socket_setsockopt=%u "
      "scenario_udp_setsockopt_reuseaddr=%u "
      "scenario_udp_setsockopt_recv_timeout=%u "
      "scenario_udp_setsockopt_send_timeout=%u scenario_udp_sendto_only=%u "
      "udp_sendto_only_timeouts=%u scenario_udp_connect_send_only=%u "
      "scenario_udp=%u scenario_tcp_multi=%u scenario_wgnx_raw_udp=%u "
      "scenario_wgnx_tunnel_udp=%u",
      ...
      static_cast<unsigned>(config::EnableScenarioWgnxTunnelUdpWorkload));
  logger::Log(ctx,
              "wgnx_packet_config api=%u source=%s:%u destination=%s:%u "
              "timeout_ms=%u poll_ms=%u malformed_ipv4_checksum=%u",
              wgnx::IpcApiVersion, config::WgnxTunnelSourceIpv4,
              config::WgnxUdpSourcePort, config::WgnxUdpEchoDestinationIpv4,
              config::WgnxUdpEchoDestinationPort, config::WgnxPacketTimeoutMs,
              config::WgnxPacketPollIntervalMs,
              static_cast<unsigned>(config::WgnxSubmitMalformedIpv4Checksum));
  logger::Log(ctx,
              "wgnx_tunnel_config api=%u destination=%s:%u workload=%u "
              "payload=%zu datagrams=%u pacing_ms=%u flows=%u deadline_ms=%u "
              "seed=0x%08x echo=%u",
              wgnx::tunnel::TunApiVersion, config::WgnxTunnelDestinationIpv4,
              config::WgnxTunnelDestinationPort, config::WgnxTunnelWorkloadId,
              config::WgnxTunnelPayloadBytes, config::WgnxTunnelDatagramCount,
              config::WgnxTunnelPacingMs, config::WgnxTunnelConcurrentFlows,
              config::WgnxTunnelReceiveDeadlineMs,
              config::WgnxTunnelPayloadSeed,
              static_cast<unsigned>(config::WgnxTunnelEchoReplies));
  logger::Log(ctx,
              "socket_config mode=%s tcp_tx=%u tcp_rx=%u tcp_tx_max=%u "
              "tcp_rx_max=%u udp_tx=%u udp_rx=%u sb_efficiency=%u "
              "num_bsd_sessions=%u service_type=%u",
              is_application ? "application" : "applet",
              socket_config.tcp_tx_buf_size, socket_config.tcp_rx_buf_size,
              socket_config.tcp_tx_buf_max_size,
              socket_config.tcp_rx_buf_max_size, socket_config.udp_tx_buf_size,
              socket_config.udp_rx_buf_size, socket_config.sb_efficiency,
  ...
  if (config::EnableAppletExitLock) {
    ...
    if (R_SUCCEEDED(rc)) {
      ...
      logger::Status(ctx, "appletLockExit failed: %s",
  ...
  if (config::EnableSocketInitialize) {
    ...
    logger::Bootstrap("main: socketInitialize rc=%s",
                      FormatResult(ctx.socket_initialize_rc).c_str());
    logger::Log(ctx, "socketInitialize(custom) -> %s",
                FormatResult(ctx.socket_initialize_rc).c_str());
    if (R_FAILED(ctx.socket_initialize_rc)) {
      logger::Status(ctx, "socketInitialize(custom) failed: %s",
                     FormatResult(ctx.socket_initialize_rc).c_str());
  ...
  if (config::EnableNifmInitialize) {
    ...
    logger::Bootstrap("main: nifmInitialize rc=%s",
                      FormatResult(ctx.nifm_initialize_rc).c_str());
    logger::Log(ctx, "nifmInitialize(User) -> %s",
                FormatResult(ctx.nifm_initialize_rc).c_str());
    if (R_FAILED(ctx.nifm_initialize_rc)) {
      logger::Status(ctx, "nifmInitialize(User) failed: %s",
                     FormatResult(ctx.nifm_initialize_rc).c_str());
  ...
  if (config::EnableSslInitialize) {
    ...
    logger::Bootstrap("main: sslInitialize rc=%s",
                      FormatResult(ctx.ssl_initialize_rc).c_str());
    logger::Log(ctx, "sslInitialize(3) -> %s",
                FormatResult(ctx.ssl_initialize_rc).c_str());
  } else {
  ...
  if (config::EnableCurlInitialize) {
    ...
    logger::Bootstrap("main: curl_global_init rc=%d (%s)",
                      static_cast<int>(ctx.curl_global_rc),
                      curl_easy_strerror(ctx.curl_global_rc));
    logger::Log(ctx, "curl_global_init(CURL_GLOBAL_DEFAULT) -> %d (%s)",
                static_cast<int>(ctx.curl_global_rc),
                curl_easy_strerror(ctx.curl_global_rc));
  } else {
  ...
  if (ctx.curl_initialized && config::EnableCurlShare) {
    ...
    logger::Log(ctx, "curl_share_init -> %p",
                static_cast<void *>(ctx.curl_share));
    if (ctx.curl_share != nullptr) {
      static_cast<void>(curl_share_setopt(ctx.curl_share, CURLSHOPT_SHARE,
  ...
  if (has_failed_scenario) {
    logger::Status(ctx,
                   "Requester finished with failures; exiting in %u seconds",
                   config::ExitDelayMs / 1000U);
  } else {
