...
BsdPollObservation ClassifyBsdPollObservation(const int poll_result,
                                              const short revents) {
  if (poll_result < 0) {
...
bool IsExpectedBsdPollObservation(const BsdSystemUdpExpectedOutcome expected,
                                  const BsdPollObservation observation) {
  switch (expected) {
...
bool HasWritableRecovery(const std::uint32_t queue_full_retries,
                         const bool observed_pollout) {
