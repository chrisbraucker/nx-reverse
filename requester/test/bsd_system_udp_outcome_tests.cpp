...
int main() {
  ...
  assert(ClassifyBsdPollObservation(1, POLLIN | POLLHUP) ==
         BsdPollObservation::Closed);
  assert(ClassifyBsdPollObservation(1, POLLOUT) ==
         BsdPollObservation::Unexpected);
  assert(IsExpectedBsdPollObservation(BsdSystemUdpExpectedOutcome::EchoReply,
                                      BsdPollObservation::Readable));
  assert(
      IsExpectedBsdPollObservation(BsdSystemUdpExpectedOutcome::NoReplyTimeout,
                                   BsdPollObservation::Timeout));
  assert(
      IsExpectedBsdPollObservation(BsdSystemUdpExpectedOutcome::TerminalClosure,
                                   BsdPollObservation::Closed));
